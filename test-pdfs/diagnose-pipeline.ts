...
async function main() {
  ...
  // Phase 2: Analyze pages (now with pdfjs fallback support)
  const scenes: PageScene[] = [];
  for (let pageIdx = 0; pageIdx < pdfJsDoc.numPages; pageIdx++) {
    const page = await pdfJsDoc.getPage(pageIdx + 1);
    // Ensure operator list is processed so page.objs has decoded images
  ...
  // Collect styles from text elements using registerRun
  for (const scene of scenes) {
    for (const el of scene.elements) {
      if (el.kind === 'text') {
        const te = el as any;
        styleCollector.registerRun(te.fontName, Math.round(te.fontSize * 2), te.bold, te.italic, te.color);
      }
    }
  }
